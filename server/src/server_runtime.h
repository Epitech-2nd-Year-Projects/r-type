...
class ServerRuntime {
 ...
 private:
   ...
   */
  void SendServerCommand(PeerConnection& peer, std::uint16_t command_id,
   ...
   */
  void SendReject(PeerConnection& peer, protocol::JoinRejectReason reason,
  ...
  /**
   * @brief Applies ACK information from the remote header to the reliable
   * queue.
   ...
   */
  void DisconnectPeer(PeerConnection& peer, std::string_view reason,
                      bool notify_client);
   ...
   * @param difficulty Difficulty level for the room.
   * @return Reference to the created room instance.
   */
  Room& CreateRoom(const std::string& room_code, const std::string& room_name,
  ...
  ServerTransport
      transport_;  ///< UDP transport facade for non-blocking send/recv.
  ServerConfig
      config_;  ///< Server configuration (port, tick rate, limits, etc.).
  engine::util::Logger& logger_;  ///< Logger instance for diagnostic output.
  engine::time::FrameTimer
      frame_timer_;  ///< Timer for maintaining fixed tick rate.
  const std::size_t worker_count_{
      1};  ///< Number of worker threads for room simulation.
  asio::thread_pool
      worker_pool_;  ///< Pool executing per-room updates in parallel.
  std::uint32_t next_player_id_{
      1};  ///< Next available player ID for assignment.
  std::unordered_map<std::string, PeerConnection>
      peers_;  ///< Map of endpoint keys to peer connections.
  std::unordered_map<std::uint32_t, PlayerSession>
      players_;       ///< Map of player IDs to player sessions.
  std::mt19937 rng_;  ///< Random number generator for deterministic seeds.
  std::unordered_map<std::string, Room>
      rooms_;                      ///< Active room contexts keyed by room code.
  std::uint32_t next_room_id_{1};  ///< Counter for room identifiers.
  std::uint32_t server_tick_{
      0};  ///< Current server tick counter since startup.
  engine::time::TimeDelta
      fixed_delta_;  ///< Fixed simulation timestep (1.0 / tick_rate).
  engine::time::TimeDelta
      accumulator_;      ///< Accumulates frame time for fixed-step simulation.
  bool running_{false};  ///< Whether the server loop is currently running.
  protocol::DecodeMetrics
      decode_metrics_{};  ///< Tracks packet decode statistics (success/error
                          ///< counts).
  std::uint32_t last_decode_metrics_log_ms_{
      0};  ///< Last timestamp when decode metrics were logged.
  std::uint32_t last_server_stats_log_ms_{
      0};  ///< Last timestamp when server stats were logged.
  std::uint32_t last_tick_health_sample_ms_{
      0};  ///< Start timestamp for the tick health window.
  std::uint32_t last_tick_health_sample_tick_{
      0};  ///< Tick counter at start of the health window.
  double frame_time_accumulator_ms_{
      0.0};  ///< Accumulated frame durations for health averages.

  std::uint64_t frame_time_samples_{
