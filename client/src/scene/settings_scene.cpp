...
SettingsScene::SettingsScene(Application& app) : app_(app) {
    ...
    ui_elements_.push_back(std::make_shared<ui::Label>(
        engine::math::Vector2f{center_x - 140.0f, 100.0f}, "Settings", 60.0f, white));
    ...
    ui_elements_.push_back(std::make_shared<ui::Label>(
        engine::math::Vector2f{center_x - 350.0f, start_y + 10.0f}, "Music Volume", 24.0f, white));
    
    ui_elements_.push_back(std::make_shared<ui::Button>(
        engine::math::Vector2f{center_x - 80.0f, start_y}, engine::math::Vector2f{40.0f, 40.0f}, "-",
        [this]() {
            if (auto audio = app_.GetEngine().Audio()) {
                float v = audio->GetMusicVolume();
                v = std::max(0.0f, v - 0.1f);
                audio->SetMusicVolume(v);
                if (music_volume_label_) music_volume_label_->SetText(VolumeToString(v));
            }
        }));

    music_volume_label_ = std::make_shared<ui::Label>(
        engine::math::Vector2f{center_x - 10.0f, start_y + 10.0f}, VolumeToString(current_music_vol), 24.0f, white);
    ui_elements_.push_back(music_volume_label_);

    ui_elements_.push_back(std::make_shared<ui::Button>(
        engine::math::Vector2f{center_x + 60.0f, start_y}, engine::math::Vector2f{40.0f, 40.0f}, "+",
        [this]() {
            if (auto audio = app_.GetEngine().Audio()) {
                float v = audio->GetMusicVolume();
    ...
    ui_elements_.push_back(std::make_shared<ui::Label>(
        engine::math::Vector2f{center_x - 350.0f, start_y + 10.0f}, "SFX Volume", 24.0f, white));
    ...
    sfx_volume_label_ = std::make_shared<ui::Label>(
        engine::math::Vector2f{center_x - 10.0f, start_y + 10.0f}, VolumeToString(current_sfx_vol), 24.0f, white);
    ...
    ui_elements_.push_back(std::make_shared<ui::Button>(
        ...
        "Toggle Fullscreen",
        [this]() {
            app_.GetEngine().Window().ToggleFullscreen();
        }));

    auto back_btn = std::make_shared<ui::Button>(
        engine::math::Vector2f{center_x - 200.0f, 750.0f}, engine::math::Vector2f{400.0f, 50.0f},
        "Back",
        [this]() {
            app_.OnQuitToMenu();
        });
    ...
    if (btn_tex) {
        ...
        back_btn->SetColors(white, engine::render::Color::FromBytes(220, 220, 220), engine::render::Color::FromBytes(180, 180, 180));
    }
    ...
    for (auto& elem : ui_elements_) {
        if (auto btn = std::dynamic_pointer_cast<ui::Button>(elem)) {
            ...
            if (btn->GetText() == "Toggle Fullscreen") {
                 ...
                 btn->SetColors(white, engine::render::Color::FromBytes(220, 220, 220), engine::render::Color::FromBytes(180, 180, 180));
            }
            if (btn->GetText() == "+" || btn->GetText() == "-") {
                 if (small_btn_tex) btn->SetTexture(small_btn_tex);
                 else if (btn_tex) btn->SetTexture(btn_tex); 
                 btn->SetColors(white, engine::render::Color::FromBytes(220, 220, 220), engine::render::Color::FromBytes(180, 180, 180));
            }
        }
    }
