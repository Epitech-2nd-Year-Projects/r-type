...
namespace {
engine::render::Color FromBytes(uint8_t r, uint8_t g, uint8_t b,
                                uint8_t a = 255) {
...
void PingWheel::Update(engine::input::InputManager& input,
  ...
  engine::math::Vector2f mouse_vec = {mouse_pos.x - center_pos_.x,
                                      mouse_pos.y - center_pos_.y};
  ...
  float step =
      2.0f * std::numbers::pi_v<float> / static_cast<float>(options_.size());
  float shifted_angle = angle + step / 2.0f;
  if (shifted_angle >= 2.0f * std::numbers::pi_v<float>)
    shifted_angle -= 2.0f * std::numbers::pi_v<float>;
...
void PingWheel::Draw(engine::render::Renderer2D& renderer) {
  ...
  renderer.DrawRing(center_pos_, kWheelRadius, kWheelRadius + 8.0f, 0, 360, 64,
                    FromBytes(100, 180, 255, 50));
  renderer.DrawRing(center_pos_, kWheelRadius + 4.0f, kWheelRadius + 14.0f, 0,
                    360, 64, FromBytes(100, 180, 255, 25));
  ...
  for (size_t i = 0; i < options_.size(); ++i) {
    ...
    engine::render::Color sector_color = {
        opt.color.r * 0.35f, opt.color.g * 0.35f, opt.color.b * 0.35f, 0.75f};
  ...
  for (size_t i = 0; i < options_.size(); ++i) {
    float rad = (static_cast<float>(i) * step_deg - step_deg / 2.0f) *
    ...
    renderer.DrawLine(line_start, line_end, 2.0f,
                      FromBytes(255, 255, 255, 100));
  }

  renderer.DrawCircle(center_pos_, kInnerRadius + 3.0f,
                      FromBytes(15, 18, 25, 250));
  renderer.DrawCircle(center_pos_, kInnerRadius - 2.0f,
                      FromBytes(25, 30, 40, 255));

  renderer.DrawRing(center_pos_, kWheelRadius - 2.0f, kWheelRadius, 0, 360, 64,
                    FromBytes(255, 255, 255, 120));
  renderer.DrawRing(center_pos_, kInnerRadius, kInnerRadius + 3.0f, 0, 360, 64,
                    FromBytes(255, 255, 255, 100));
  ...
  for (size_t i = 0; i < options_.size(); ++i) {
    ...
    if (is_selected) {
      renderer.DrawCircle(dot_pos, dot_radius + 3.0f,
                          opt.color.WithAlpha(0.4f));
    }
