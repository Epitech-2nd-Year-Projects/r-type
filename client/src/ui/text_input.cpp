...
char KeyToChar(engine::input::Key key, bool shift) {
  ...
  if (key >= Key::kA && key <= Key::kZ) {
    ...
    else if (key == Key::kM) return shift ? '?' : ','; // QWERTY M -> AZERTY ,
...
TextInput::TextInput(engine::math::Vector2f position, engine::math::Vector2f size)
...
void TextInput::Update(engine::time::TimeDelta dt, engine::input::InputManager& input) {
  if (focused_) {
...
void TextInput::Draw(engine::render::Renderer2D& renderer) {
  renderer.DrawRect({position_.x, position_.y, size_.x, size_.y}, background_color_);
  
  engine::render::Color border = focused_ ? focused_border_color_ : border_color_;
  renderer.DrawLine(position_, {position_.x + size_.x, position_.y}, 2.0f, border);
  renderer.DrawLine({position_.x + size_.x, position_.y}, {position_.x + size_.x, position_.y + size_.y}, 2.0f, border);
  renderer.DrawLine({position_.x + size_.x, position_.y + size_.y}, {position_.x, position_.y + size_.y}, 2.0f, border);
  renderer.DrawLine({position_.x, position_.y + size_.y}, position_, 2.0f, border);
  ...
  renderer.DrawText(display_text, {position_.x + 10.0f, text_y}, font_size, color);

  if (focused_ && show_cursor_) {
    if (!text_.empty()) {
       renderer.DrawText(display_text + "|", {position_.x + 10.0f, text_y}, font_size, color);
    } else {
       renderer.DrawText("|", {position_.x + 10.0f, text_y}, font_size, text_color_);
    }
  }
}
...
void TextInput::HandleTyping(engine::input::InputManager& input) {
  ...
  if (input.IsKeyDown(Key::kBackspace)) {
      ...
      if (!last_key_states_[key_idx]) {
          if (!text_.empty()) {
              text_.pop_back();
              engine::util::Logger::Default().Info("TextInput: Backspace. Current text: ", text_);
          }
      }
  ...
  auto check_key = [&](Key key) {
      ...
      if (idx < 0 || static_cast<std::size_t>(idx) >= last_key_states_.size()) return;
      ...
      if (down && !last_key_states_[idx]) {
          ...
          if (c != 0) {
              text_ += c;
              engine::util::Logger::Default().Info("TextInput: Added char '", c, "'. Current text: ", text_);
          }
      }
  ...
  for (int k = static_cast<int>(Key::kNum0); k <= static_cast<int>(Key::kNum9); ++k) {
      check_key(static_cast<Key>(k));
  }
