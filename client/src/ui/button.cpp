...
Button::Button(engine::math::Vector2f position, engine::math::Vector2f size, const std::string& text, std::function<void()> on_click)
    : position_(position), size_(size), text_(text), on_click_(on_click) {}

void Button::Update(engine::time::TimeDelta /*dt*/, engine::input::InputManager& input) {
...
void Button::Draw(engine::render::Renderer2D& renderer) {
    ...
    if (pressed_) color = pressed_color_;
    else if (hovered_) color = hover_color_;
    
    if (texture_) {
        engine::render::SpriteDrawParams params;
        params.position = position_;
        params.scale = {size_.x / static_cast<float>(texture_->GetSize().x), size_.y / static_cast<float>(texture_->GetSize().y)};
        params.tint = color;
...
void Button::SetColors(engine::render::Color normal, engine::render::Color hover, engine::render::Color pressed) {
