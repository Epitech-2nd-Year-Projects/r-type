...
class CollisionSystem : public engine::ecs::ISystem {
 ...
 private:
  ...
  /**
   * @brief Check collision between compound circle collider and AABB
   */
  bool CheckCompoundCircleVsAABB(
      const engine::ecs::CompoundCircleColliderComponent &compound,
      const engine::math::Vector2f &compound_pos,
      const engine::math::RectF &box, const engine::math::Vector2f &box_pos);

  /**
   * @brief Check collision between two compound circle colliders
   */
  bool CheckCompoundCircleVsCompoundCircle(
