...
/**
 * @brief Component attached to a player to trigger a shoot event with lag
 * compensation.
 *
 * If 'fired' is true, the weapon system will spawn a projectile.
 * 'spawn_position' is where the player WAS when they pressed the button
 * (server-rewound). 'latency_s' is how much time has passed since the shot (to
 * fast-forward the projectile).
 */
