...
std::vector<engine::net::PacketBuffer> SplitPacketBuffer(
  ...
  const std::size_t frag_info_size = sizeof(std::uint16_t) + sizeof(std::uint8_t) * 2;
  ...
  for (std::uint8_t i = 0; i < fragment_count; ++i) {
      ...
      frag_buf.write_bytes(std::span<const std::uint8_t>(raw.data() + offset, chunk_size));
...
void FragmentReassembler::Cleanup(std::uint32_t now_ms, std::uint32_t timeout_ms) {
