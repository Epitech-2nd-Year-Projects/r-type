...
ParallaxCamera::ParallaxCamera() noexcept { UpdatePixelsPerUnit(); }

ParallaxCamera::ParallaxCamera(const math::Vector2f& viewport_size,
                               float vertical_min, float vertical_max) noexcept
...
void ParallaxCamera::SetViewportSize(const math::Vector2f& size) noexcept {
  viewport_size_.x = std::max(size.x, kMinimumViewportExtent);
  viewport_size_.y = std::max(size.y, kMinimumViewportExtent);
  UpdatePixelsPerUnit();
}

void ParallaxCamera::SetVerticalRange(float min_y, float max_y) noexcept {
  vertical_min_ = min_y;
  vertical_max_ = std::max(max_y, min_y + kMinimumWorldHeight);
  UpdatePixelsPerUnit();
}

math::Vector2f ParallaxCamera::WorldToScreen(const math::Vector2f& world,
...
math::Vector2f ParallaxCamera::GetViewSizeWorld() const noexcept {
...
math::RectF ParallaxCamera::GetViewRectWorld() const noexcept {
  const math::Vector2f view_size = GetViewSizeWorld();
  const float half_width = view_size.x * 0.5f;
...
void ParallaxCamera::UpdatePixelsPerUnit() noexcept {
  const float world_height =
      ClampExtent(vertical_max_ - vertical_min_, kMinimumWorldHeight);
