...
std::uint8_t BuildButtonMask(const rift::client::FightActionState& state) {
...
InputSender::InputSender(FightInputLayer& input_layer,
...
void InputSender::SetSendRateHz(float hz) {
  const float clamped = std::clamp(hz, 30.0f, 120.0f);
