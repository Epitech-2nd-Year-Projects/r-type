...
ServerConfig LoadServerConfig() {
  ...
  apply_uint("RIFT_SERVER_PORT", kMinPort, kMaxPort, config.port);
  apply_uint("RIFT_SERVER_MAX_PLAYERS", kMinPlayers, kMaxPlayers,
             config.max_players);
  apply_uint("RIFT_SERVER_TICK_RATE", kMinTickRate, kMaxTickRate,
             config.tick_rate);
  apply_uint("RIFT_SERVER_TIMEOUT_MS", kMinPeerTimeoutMs, kMaxPeerTimeoutMs,
             config.peer_timeout_ms);
  apply_uint("RIFT_SERVER_ROOM_IDLE_TIMEOUT_MS", kMinRoomIdleTimeoutMs,
             kMaxRoomIdleTimeoutMs, config.room_idle_timeout_ms);

  if (const char* seed_value = std::getenv("RIFT_SERVER_SEED")) {
    std::uint32_t seed = 0;
  ...
  if (const char* room_code = std::getenv("RIFT_SERVER_ROOM_CODE")) {
  ...
  if (const char* room_name = std::getenv("RIFT_SERVER_ROOM_NAME")) {
  ...
  if (const char* log_level = std::getenv("RIFT_SERVER_LOG_LEVEL")) {
