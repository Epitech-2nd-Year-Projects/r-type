...
// - To customize capacities, use SetMediaFlag(MEDIA_VIDEO_QUEUE,
// [videoQueueCapacity])
...
// Buffer structure
// - A single buffer is used to hold decoded audio before feeding it to the
// AudioStream.
// - Set the buffer capacity for a specific MediaStream before calling
// LoadMedia(),
//   or rely on the default value if not set.
// - To customize capacity, use SetMediaFlag(MEDIA_AUDIO_DECODED_BUFFER,
// [decodedBufferCapacity]).
...
// - Changes to these settings will take effect only on MediaStreams loaded
// after the change.
typedef struct MediaConfig {
  ...
  int audioDecodedBufferSize;  // Size in bytes of the buffer holding decoded
                               // audio for the AudioStream
  ...
  double maxAllowedDelay[STREAM_COUNT];  // Maximum allowed delay (in seconds)
                                         // before an Audio or Video packet is
                                         // discarded Late packets below this
                                         // threshold are speed-up

  int audioMaxUpdateSize;     // Maximum number of bytes to be uploaded to the
                              // AudioStream in a frame
...
typedef struct StreamDataContext {
  ...
  PacketQueue pendingPackets;  // Queue of pending packets, enqueued if they
                               // cannot be used immediately
  int streamIdx;  // Index of this stream within the AVFormatContext structure
  int64_t startPts;  // Starting presentation timestamp (PTS) of the stream;
                     // AV_NOPTS_VALUE initially
} StreamDataContext;
...
typedef struct MediaContext {
  ...
  StreamDataContext
      streams[STREAM_COUNT];  // Data associated with video/audio streams

  // Video stream-related fields
  struct SwsContext* swsContext;  // Video resampling and scaling context
  Image videoOutputImage;      // Image buffer holding the decoded video frame,
                               // uploaded to [MediaStream].videoTexture
  uint8_t* alignedRgbData[4];  // Pointers to the RGB buffers used for frame
                               // conversion
  int alignedRgbLinesize[4];   // Linesizes for the RGB buffers used for frame
                               // conversion

  // Audio stream-related fields
  struct SwrContext* swrContext;  // Audio resampling context
  Buffer audioOutputBuffer;       // Buffer with decoded audio, used to fill the
                                  // AudioStream when needed
  int audioOutputFmt;      // Output audio format for this stream; must be an
                           // interleaved format
  int audioMaxUpdateSize;  // Maximum number of bytes to be uploaded to the
                           // AudioStream in a frame

  // libav* library-related fields
  AVPacket* avPacket;  // AVPacket used before dispatching to the correct stream
                       // context
  AVFrame* avFrame;    // AVFrame used for each packet during processing

  // MediaStream-related fields
  MediaState state;  // Current state of the media. Use
                     // SetMediaState()/GetMediaState() to modify.
  double timePos;    // Current playback position in seconds
  bool loopPlay;     // Indicates if the media plays in a loop. Use
                     // SetMediaLooping() to set.
} MediaContext;
...
// Default global settings in raylib style (see documentation for MediaConfig,
// SetMediaFlag(), and GetMediaFlag()).
static MediaConfig MEDIA = {
    ...
    .maxAllowedDelay =
        {0.04,
         1.0}  //! IMPORTANT: Assuming here STREAM_AUDIO = 0, STREAM_VIDEO = 1
};
...
int IsBufferFull(
    const BufferState* state);  // Check if the circular buffer is full (returns
                                // 1 if full, 0 otherwise)
int IsBufferEmpty(
    const BufferState* state);  // Check if the circular buffer is empty
                                // (returns 1 if empty, 0 otherwise)

// Space calculation functions

int GetBufferWritableSpace(
    const BufferState* state);  // Calculate total writable space in the buffer,
                                // in bytes (not necessarily contiguous)
int GetBufferWritableSegmentSize(
    const BufferState*
        state);  // Calculate the largest contiguous writable segment, in bytes
int GetBufferReadableSpace(
    const BufferState* state);  // Calculate total readable space in the buffer,
                                // in bytes (not necessarily contiguous)
int GetBufferReadableSegmentSize(
    const BufferState*
        state);  // Calculate the largest contiguous readable segment, in bytes

// Position advancing functions

void AdvanceWritePos(BufferState* state);  // Advance the write position by one
                                           // byte in the circular buffer
void AdvanceWritePosN(
    BufferState* state,
    int n);  // Advance the write position by N bytes in the circular buffer
void AdvanceReadPos(BufferState* state);  // Advance the read position by one
                                          // byte in the circular buffer
void AdvanceReadPosN(
    BufferState* state,
    int n);  // Advance the read position by N bytes in the circular buffer
...
Buffer LoadBuffer(
    int capacity);  // Load a circular buffer with the specified capacity.
void UnloadBuffer(Buffer* buffer);  // Free memory associated with the buffer.
bool IsBufferReady(
    const Buffer* buffer);         // Check if the buffer is properly loaded.
void ClearBuffer(Buffer* buffer);  // Reset the circular buffer without freeing
                                   // memory, allowing for reuse.

int WriteBuffer(
    Buffer* buffer, const uint8_t* srcData,
    int srcSize);  // Write srcData to the buffer. Returns a negative value on
                   // error; otherwise, returns the actual size written (may be
                   // less than srcSize if not enough writable space is
                   // available).

int ReadBuffer(Buffer* buffer, uint8_t* dstData,
               int dstSize);  // Read data from the buffer into dstData. Returns
                              // a negative value on error; otherwise, returns
                              // the actual size read (may be less than dstSize
...
PacketQueue LoadQueue(
    int capacity);  // Load a packet queue with the specified capacity.
void UnloadQueue(PacketQueue* queue);  // Free memory associated with the queue.
bool IsQueueReady(
    const PacketQueue* queue);        // Check if the queue is properly loaded.
void ClearQueue(PacketQueue* queue);  // Reset the queue without freeing memory,
                                      // allowing for reuse.
bool IsQueueFull(const PacketQueue* queue);   // Check if the queue is full (no
                                              // more writable space).
bool IsQueueEmpty(const PacketQueue* queue);  // Check if the queue is empty (no
                                              // more readable space).

bool EnqueuePacket(PacketQueue* queue,
                   AVPacket* src);  // Enqueue a packet. Automatically handles
                                    // reference management. Returns true on
                                    // success, false if the queue is full.

bool DequeuePacket(PacketQueue* queue,
                   AVPacket* dst);  // Dequeue a packet. Automatically handles
                                    // reference management. Returns true on
                                    // success, false if the queue is empty.

AVPacket* PeekPacket(
    const PacketQueue*
        queue);  // Returns a pointer to the first available packet, or NULL if
                 // the queue is empty. Does not modify the queue or advance the
                 // read position.
...
// First, searches the pending packets queue; if empty, it grabs and enqueues
// packets until finding one of the desired type.
int AVGrabPacket(MediaContext* ctx, int streamType, AVPacket* dst);

// Returns a ptr to a packet of the desired type keeping the reference inside
// the queue
int AVPeekPacket(MediaContext* ctx, int streamType, AVPacket** ptr);

// Decodes a packet grabbed with AVGrabPacket, then calls AVProcessVideoFrame or
// AVProcessAudioFrame based on the packet type. Optionally discards the packet
// without processing it.
int AVDecodePacket(const MediaStream* media, int streamType,
                   const AVPacket* packet, bool discardPacket);

// Processes a specific audio frame to provide usable data for
// [MediaStream].audioStream.
int AVProcessAudioFrame(const MediaStream* media);

// Processes a specific video frame to provide usable data for
// [MediaStream].videoTexture.
int AVProcessVideoFrame(const MediaStream* media);

// Helper for seeking to the first video keyframe in the media. Called by AVSeek
// after codec are flushed. It's used to avoid visual codec artifacts while
// seeking in the media.
bool AVSeekVideoKeyframe(const MediaStream* media);

// Helper for seeking to a specific position in the media (targetTimestamp in
// libav time units).
bool AVSeek(MediaStream* media, int64_t targetTimestamp);

// Helper for relative position seeking in the media (factor is the relative
// position between 0.0 and 1.0) [not used].
bool AVSeekRelative(MediaStream* media, double factor);

// Helper function to load codec context data for a specific stream.
bool AVLoadCodecContext(StreamDataContext* streamCtx, const AVCodec* codec,
                        const AVCodecParameters* params);

// Helper function to free memory associated with codec context data for a
// specific stream.
...
// - fileName: Name of the media file to load. Ignored if a valid
// MediaStreamReader is provided.
// - streamReader: A MediaStreamReader containing custom IO callbacks. Takes
// precedence over fileName if valid.
// - flags: Combination of MediaLoadFlag values to configure loading behavior.
// Returns: Pointer to the allocated MediaContext on success, or NULL on
// failure.
MediaContext* LoadMediaContext(const char* fileName,
                               MediaStreamReader streamReader, int flags);
...
// This internal function is shared by exposed API functions (LoadMedia,
// LoadMediaEx, LoadMediaFromStream), which differ only in how the MediaContext
// is initialized.
// - ctx: Pointer to an initialized MediaContext structure.
// - flags: Combination of MediaLoadFlag values to configure the loading
// process. Returns: A valid MediaStream on success; an empty MediaStream on
// failure.
MediaStream LoadMediaFromContext(MediaContext* ctx, int flags);

void NotifyEndOfStream(
    const MediaStream*
        media);  // Handles the end-of-stream event for the specified media.

void UpdateState(const MediaStream* media,
                 int newState);  // Helper function to update the state of the
                                 // media to the specified new state.

bool HasStream(const MediaContext* ctx,
               int streamType);  // Checks if the media has an available
                                 // VIDEO_STREAM or AUDIO_STREAM.
...
int SetMediaFlag(int flag, int value) {
  ...
  switch (flag) {
    ...
    case MEDIA_AUDIO_FORMAT:
      // A planar format (not interleaved) would require a different
      // implementation
      if (!av_sample_fmt_is_planar(value)) {
        MEDIA.audioOutputFmt = (enum AVSampleFormat)CLAMP(
            value, AV_SAMPLE_FMT_U8, AV_SAMPLE_FMT_DBL);
      } else {
        TraceLog(LOG_WARNING,
                 "MEDIA: Non-interleaved audio format (%i) is not supported.",
                 value);
      }
      break;
...
MediaProperties GetMediaProperties(MediaStream media) {
  ...
  if (IsMediaValid(media)) {
    ...
    props.durationSec =
        ...
        AV_TIME_BASE;  // av_rescale_q(fmtCtx->duration, AV_TIME_BASE_Q,
                       // (AVRational) { 1, 1000 });
    ...
    props.avgFPS =
        props.hasVideo
            ? (float)av_q2d(
                  fmtCtx->streams[videoCtx->streamIdx]->avg_frame_rate)
            : 0;
  } else {
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to retrieve properties of an invalid media.");
  }
...
bool SetMediaPosition(MediaStream media, double timeSec) {
  if (!IsMediaValid(media)) {
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to set the position of an invalid media.");
    return false;
  }
...
double GetMediaPosition(MediaStream media) {
  ...
  if (IsMediaValid(media)) {
    ...
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to retrieve the position of an invalid media.");
  }
...
bool SetMediaLooping(MediaStream media, bool loopPlay) {
  ...
  if (IsMediaValid(media)) {
    ...
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to set the looping mode of an invalid media.");
  }
...
MediaContext* LoadMediaContext(const char* fileName,
  ...
  *ctx = (MediaContext){0};
  ...
  for (int i = 0; i < (int)ctx->formatContext->nb_streams; i++) {
    const AVCodecParameters* localCodecParameters =
        ctx->formatContext->streams[i]->codecpar;

    const AVCodec* localCodec =
        avcodec_find_decoder(localCodecParameters->codec_id);
    ...
    if (localCodecParameters->codec_type == AVMEDIA_TYPE_VIDEO &&
      ...
      if (ret == MEDIA_RET_SUCCEED) {
        ...
        if (!IsQueueReady(&videoCtx->pendingPackets)) {
          TraceLog(LOG_ERROR,
                   "MEDIA: Cannot initialize the video packet queue.");
        ...
        ctx->swsContext = sws_getContext(codecCtx->width, codecCtx->height,
        ...
        ctx->videoOutputImage.data = RL_MALLOC(av_image_get_buffer_size(
            AV_PIX_FMT_RGB24, codecCtx->width, codecCtx->height, 1));

        if (!ctx->videoOutputImage.data) {
          TraceLog(
              LOG_ERROR,
              "MEDIA: Cannot allocate memory for holding the decoded frame.");
        ...
        int allocatedBytes = av_image_alloc(
            ctx->alignedRgbData, ctx->alignedRgbLinesize, codecCtx->width,
            codecCtx->height, AV_PIX_FMT_RGB24, 32);

        if (allocatedBytes < 0) {
          TraceLog(
              LOG_ERROR,
              "MEDIA: Cannot allocate memory for the frame conversion buffer.");
        ...
        TraceLog(LOG_WARNING,
                 "MEDIA: Cannot initialize the video codec. (Error code: %i)",
                 ret);
      }

      TraceLog(LOG_DEBUG, "Media Debug: '%s' - Video Codec: resolution %d x %d",
               fileName, localCodecParameters->width,
               localCodecParameters->height);
    } else if (localCodecParameters->codec_type == AVMEDIA_TYPE_AUDIO &&
               (flags & MEDIA_LOAD_NO_AUDIO) == 0) {
      if (!IsAudioDeviceReady()) {
        TraceLog(LOG_WARNING,
                 "MEDIA: '%s' - Audio Codec: raylib audio device is not "
                 "initialized. Audio will be skipped.",
                 fileName);
        continue;
      }
      ...
      if (ret == MEDIA_RET_SUCCEED) {
        ...
        if (!IsQueueReady(&audioCtx->pendingPackets)) {
          TraceLog(LOG_ERROR,
                   "MEDIA: Cannot initialize the audio packet queue.");
        ...
        if (!IsBufferReady(&ctx->audioOutputBuffer)) {
          TraceLog(
              LOG_ERROR,
              "MEDIA: Cannot allocate memory for holding the decoded audio.");
        ...
        ret = swr_alloc_set_opts2(&ctx->swrContext, &out_ch_layout,
      ...
      TraceLog(LOG_DEBUG,
               ...
               fileName, localCodecParameters->ch_layout.nb_channels,
               localCodecParameters->sample_rate);
    }

    TraceLog(LOG_DEBUG,
             "Media Debug: '%s' - Audio Codec: %s ID %d bit_rate %lld",
             fileName, localCodec->name, localCodec->id,
             localCodecParameters->bit_rate);
  }
...
MediaStream LoadMediaFromStream(MediaStreamReader streamReader, int flags) {
  if (!streamReader.readFn) {
    TraceLog(
        LOG_ERROR,
        "MEDIA: A valid read function is required to load media from a stream");
    return (MediaStream){0};
  }
...
int SetMediaState(MediaStream media, int newState) {
  ...
  if (curState == MEDIA_STATE_INVALID) {
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to change the state of an invalid media");
...
bool UpdateMediaEx(MediaStream* media, double deltaTime) {
  ...
  for (int i = 0; i < STREAM_COUNT; ++i) {
    ...
    while (discardPacketAndContinue) {
      ...
      // Peek at a packet from the queue without modifying the queue or changing
      // packet references.
      ...
      if (ret != MEDIA_RET_SUCCEED) {
        TraceLog(
            LOG_WARNING,
            "MEDIA: Failed grabbing packet from stream %i. (Error code: %i)", i,
            ret);
        break;
      }
      ...
      const AVRational* timebase =
          &ctx->formatContext->streams[streamCtx->streamIdx]->time_base;
      const double nextFrameTime =
          (double)(avPacket->pts - streamCtx->startPts) * timebase->num /
          timebase->den;
      ...
      if (ret < 0) {
        TraceLog(LOG_ERROR,
                 "MEDIA: Decoding packet (stream type: %i, error code: %i)", i,
                 ret);
      }

      // Advance the read position in the circular buffer queue and un-reference
      // the packet. This is equivalent to DequeueBuffer but avoids transferring
      // ownership of the packet reference.
  ...
  if (HasStream(ctx, STREAM_AUDIO) &&
      IsAudioStreamProcessed(media->audioStream)) {
    const int readableSegmentBytes =
        GetBufferReadableSegmentSize(&ctx->audioOutputBuffer.state);

    const int updateSize = MIN(readableSegmentBytes, ctx->audioMaxUpdateSize);

    const int bytesPerSample = (int)((media->audioStream.sampleSize / 8) *
                                     media->audioStream.channels);

    const int frameCount = updateSize / bytesPerSample;

    UpdateAudioStream(
        media->audioStream,
        &ctx->audioOutputBuffer.data[ctx->audioOutputBuffer.state.readPos],
        frameCount);
...
void AdvanceWritePos(BufferState* state) { AdvanceWritePosN(state, 1); }

void AdvanceWritePosN(BufferState* state, int n) {
  state->writePos = (state->writePos + n) % state->capacity;
}

void AdvanceReadPos(BufferState* state) { AdvanceReadPosN(state, 1); }
...
Buffer LoadBuffer(int capacity) {
  ...
  if (ret.data) {
    ...
    TraceLog(LOG_ERROR,
             "MEDIA: Failed to allocate a buffer with capacity of %i bytes.",
             capacity);
  }
...
int WriteBuffer(Buffer* buffer, const uint8_t* srcData, int srcSize) {
  ...
  while (sizeToWrite > 0) {
    const int segmentToWrite =
        MIN(sizeToWrite, GetBufferWritableSegmentSize(&buffer->state));
  ...
  if (sizeToWrite > 0) {
    TraceLog(LOG_WARNING, "MEDIA: Could not write %i bytes to the buffer.",
             sizeToWrite);
    if (IsBufferFull(&buffer->state)) {
      if (IsBufferFull(&buffer->state)) {
        TraceLog(LOG_WARNING,
                 "The buffer was filled while writing. Consider increasing its "
                 "capacity. "
...
int ReadBuffer(Buffer* buffer, uint8_t* dstData, int dstSize) {
  ...
  while (sizeToRead > 0) {
    const int segmentToRead =
        MIN(sizeToRead, GetBufferReadableSegmentSize(&buffer->state));
  ...
  if (sizeToRead > 0) {
    TraceLog(LOG_WARNING, "MEDIA: Could not read %i bytes from the buffer.",
             sizeToRead);
    if (IsBufferEmpty(&buffer->state)) {
      TraceLog(LOG_WARNING,
               "Readable data was exhausted while reading. Consider increasing "
               "buffer capacity."
...
PacketQueue LoadQueue(int capacity) {
  ...
  if (ret.packets) {
    ...
    for (int i = 0; i < capacity; ++i) {
      ...
      if (!ret.packets[i]) {
        TraceLog(LOG_ERROR,
                 "MEDIA: Failed to allocate packet at index %i, the queue will "
                 "be unloaded.",
                 i);
    ...
    TraceLog(
        LOG_ERROR,
        "MEDIA: Failed to allocate a packet queue with capacity of %i packets.",
        capacity);
  }
...
void ClearQueue(PacketQueue* queue) {
  ...
  if (queue->packets) {
    ...
    TraceLog(
        LOG_WARNING,
        "MEDIA: Trying to clear a packet queue with no allocated packets.");
  }
}
...
bool EnqueuePacket(PacketQueue* queue, AVPacket* src) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR,
             "MEDIA: Trying to enqueue packet to a queue with no allocated "
             "packets.");
    return false;
  }
...
bool DequeuePacket(PacketQueue* queue, AVPacket* dst) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR,
             "MEDIA: Trying to dequeue packet from a queue with no allocated "
             "packets.");
    return false;
  }
...
AVPacket* PeekPacket(const PacketQueue* queue) {
  ...
  if (!queue->packets) {
    TraceLog(LOG_ERROR,
             "MEDIA: Trying to peek a packet from a queue with no allocated "
             "packets.");
    return NULL;
  }
...
bool AVInitializePts(MediaStream* media) {
  ...
  for (int i = 0; i < STREAM_COUNT; ++i) {
    ...
    if (streamCtx->startPts == AV_NOPTS_VALUE) {
      const int64_t st =
          ctx->formatContext->streams[streamCtx->streamIdx]->start_time;
      ...
      if (ret == MEDIA_EOF) {
        TraceLog(LOG_WARNING,
                 "MEDIA: Stream %d has no packets, defaulting startPts=0", i);
      ...
      if (ret != MEDIA_RET_SUCCEED || !pkt) {
        TraceLog(LOG_WARNING,
                 "MEDIA: Failed peeking packet from stream %d (err=%d)", i,
                 ret);
        return false;
      }
      ...
      if (ts == AV_NOPTS_VALUE) {
        TraceLog(LOG_WARNING,
                 "MEDIA: Stream %d packet has no valid PTS/DTS, defaulting "
                 "startPts=0",
                 i);
        ts = 0;
      }
...
int AVGrabPacket(MediaContext* ctx, int streamType, AVPacket* dst) {
  ...
  if (!ctx->streams[streamType].codecCtx) {
    TraceLog(LOG_WARNING,
             "MEDIA: Trying to grab a packet of an unavailable type: %i",
             streamType);
    return MEDIA_ERR_GRAB_PACKET;
  }

  // If there are no pending AVPackets of the desired type to dequeue, start
  // grabbing and enqueueing new packets until a packet of the desired type is
  // available.
  while (IsQueueEmpty(&ctx->streams[streamType].pendingPackets)) {
    ...
    if (ret < 0) {
      if (ret == AVERROR_EOF) {
        ...
        AVPrintError(ret);
        TraceLog(LOG_ERROR, "MEDIA: Error reading packet for stream type %d",
                 streamType);
      }
    ...
    if (dst->stream_index ==
        ctx->streams[STREAM_VIDEO]
            .streamIdx)  // The grabbed packet is a video packet
    {
      ...
      // Otherwise, enqueue this video packet since a different packet type was
      // requested
      EnqueuePacket(&ctx->streams[STREAM_VIDEO].pendingPackets, dst);
    } else if (dst->stream_index ==
               ctx->streams[STREAM_AUDIO]
                   .streamIdx)  // The grabbed packet is an audio packet
    {
      ...
      // Otherwise, enqueue this audio packet since a different packet type was
      // requested
  ...
  return DequeuePacket(&ctx->streams[streamType].pendingPackets, dst)
             ? MEDIA_RET_SUCCEED
             : MEDIA_ERR_GRAB_PACKET;
}
...
bool AVSeekRelative(MediaStream* media, double factor) {
  ...
  const int64_t targetTimestamp =
      (int64_t)((double)media->ctx->formatContext->duration * factor);
...
bool AVSeekVideoKeyframe(const MediaStream* media) {
  ...
  if (!HasStream(media->ctx, STREAM_VIDEO)) return true;
  ...
  while (true) {
    ...
    if (ret != MEDIA_RET_SUCCEED) {
      TraceLog(
          LOG_WARNING,
          "MEDIA: Failed grabbing packet from video stream. (Error code: %i)",
          ret);
      return false;
    }
    ...
    if (vPacket->flags & AV_PKT_FLAG_KEY) {
      ...
      ctx->timePos =
          (double)(vPacket->pts - streamCtx->startPts) *
          av_q2d(ctx->formatContext->streams[streamCtx->streamIdx]->time_base);
      break;
    }

    // Discard non-keyframe packets (Same of DequeuePacket, but it avoids
    // redundant reference moving)
    AdvanceReadPos(&streamCtx->pendingPackets.state);
...
bool AVSeek(MediaStream* media, int64_t targetTimestamp) {
  ...
  const int ret =
      avformat_seek_file(ctx->formatContext, -1, INT64_MIN, targetTimestamp,
  ...
  // If the media has a video stream then seek the first video keyframe to avoid
  ...
  if (IsAudioStreamValid(media->audioStream)) {
    ...
    switch (GetMediaState(*media)) {
      case MEDIA_STATE_PLAYING:
        ...
        UpdateMediaEx(media,
                      0.0);  // grab the first packets with deltaTime = 0.0
...
int AVProcessVideoFrame(const MediaStream* media) {
  ...
  int ret =
      sws_scale(ctx->swsContext, (const uint8_t* const*)ctx->avFrame->data,
                ctx->avFrame->linesize, 0, codec->height, ctx->alignedRgbData,
                ctx->alignedRgbLinesize);
  if (ret <= 0) {
...
int AVProcessAudioFrame(const MediaStream* media) {
  ...
  // until all samples are converted.
  int leftSamples = inSamples;

  do {
    if (IsBufferFull(&ctx->audioOutputBuffer.state)) {
      TraceLog(LOG_WARNING,
               "MEDIA: Not enough space for decoding in the audio buffer.");
      ret = MEDIA_ERR_OVERFLOW;
      break;
    }

    const int writableSegmentSizeBytes =
        GetBufferWritableSegmentSize(&ctx->audioOutputBuffer.state);

    // Determine the number of bytes per audio frame, based on sample size and
    // channel count.
    const int bytesPerFrame = (int)((media->audioStream.sampleSize / 8) *
                                    media->audioStream.channels);

    // Calculate the writable segment size in terms of audio samples.
    const int writableSegmentSizeSamples =
        writableSegmentSizeBytes / bytesPerFrame;

    uint8_t* outputBuffer =
        &ctx->audioOutputBuffer.data[ctx->audioOutputBuffer.state.writePos];

    // Convert and store the incoming audio samples into the output buffer.
    // This will fill up to the writable segment size in samples, using the
    // provided input data.
    const int convertedSamples =
        swr_convert(ctx->swrContext, &outputBuffer, writableSegmentSizeSamples,
                    inData, inSamples);
    ...
    // Calculate the size in bytes of the converted samples just written to the
    // buffer.
    const int convertedSamplesBytes =
        av_samples_get_buffer_size(NULL, (int)media->audioStream.channels,
                                   convertedSamples, ctx->audioOutputFmt, 1);

    AdvanceWritePosN(&ctx->audioOutputBuffer.state, convertedSamplesBytes);

    leftSamples -= convertedSamples;

    // Setting inData to NULL and inSamples to 0 instructs subsequent
    // swr_convert calls to process any remaining buffered data
    inData = NULL;
    inSamples = 0;
...
bool AVLoadCodecContext(StreamDataContext* streamCtx, const AVCodec* codec,
  ...
  if (streamCtx->codecCtx) {
    TraceLog(LOG_WARNING,
             "MEDIA: Multiple stream of the same type (%i), only the first "
             "will be used.",
             params->codec_type);
  ...
  if (!streamCtx->codecCtx) {
    TraceLog(LOG_WARNING,
             "MEDIA: Failed to allocate memory for a codec (type %i)",
             params->codec_type);
