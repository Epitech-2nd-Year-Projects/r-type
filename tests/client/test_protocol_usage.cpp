...
protocol::InputStatePayload CreatePayloadFromState(
    const client::ActionState& state, uint32_t sequence) {
...
TEST(ProtocolUsageTest, ClientInputToProtocol) {
  ...
  EXPECT_TRUE(input_layer.state().move_up)
      << "InputLayer failed to map 'Z' to MoveUp";

  protocol::InputStatePayload payload =
      CreatePayloadFromState(input_layer.state(), 101);

  EXPECT_TRUE(payload.commands[0].buttons & protocol::kInputUp)
      << "Payload failed to include kInputUp bit";

  engine::net::PacketBuffer buffer;
  ASSERT_TRUE(protocol::EncodeInputState(payload, buffer))
      << "Failed to encode input state";
  ...
  EXPECT_EQ(decoded.commands[0].buttons, protocol::kInputUp)
      << "Decoded buttons mismatch";
}
...
TEST(ProtocolUsageTest, SnapshotIntegration) {
  ...
  engine::net::PacketBuffer buffer;
  ASSERT_TRUE(protocol::EncodeWorldSnapshot(server_snapshot, buffer))
      << "Encode failed";
  ...
  EXPECT_TRUE(entity_found)
      << "Client logic would not have found the correct entity update";
}
